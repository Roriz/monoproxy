...
context.window.entityClassifications.set('azik eggers', 'person');
context.window.entityClassifications.set('arrodes', 'item');
context.window.entityClassifications.set('beyonders', 'other');
...
test('Narrative Absence Spans Chart Unit Tests', async (t) => {
  ...
  await t.test('should resolve entity properties generically (from_chapter vs start_chapter)', () => {
    ...
    assert.strictEqual(label1.textContent, 'Azik Eggers');
    assert.strictEqual(segment1.style.left, `${(82 / 500) * 100}%`);
    assert.strictEqual(segment1.style.width, `${(285 / 500) * 100}%`);
    assert.strictEqual(segment1.style.color, 'var(--color-person)');
